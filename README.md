...
) be applied? We validate our framework on Qwen3.5-4B models fine-tuned on the Shamela corpus, demonstrating its utility in guiding architectural decisions—specifically, identifying when layer duplication is necessary to expand model capacity. Our results show that Far-Layer CKA values exceeding 0.80 and Condition Numbers above 500 indicate representational saturation, warranting depth expansion. The framework is made publicly available as an open-source tool for the Arabic NLP community.
1. Introduction
